# -*- coding: utf-8 -*-
"""
Created on Wed May 13 14:03:27 2020
...
class SVMBackTest():
    ...
    def train_model(self):
        x, y = preprocess(self.model_data[:self.start_date], na_method='drop', scale_method='minmax', test_ratio=None)
        y = y.astype(np.int16)
        self.model = SVC(C=100, kernel='rbf', degree=3, gamma='scale', probability=True, class_weight='balanced')
        self.model.fit(x, y)
        
    def predict(self,cur_date):
        x, _ = preprocess(self.model_data[:cur_date], na_method='drop', scale_method='minmax',test_ratio=None)
        x = x[-1].reshape(1, -1)
